...
import type { Transaction, TransactionType } from '@/shared/store/types/transactions';
...
export function TransactionTable({ defaultType, showTypeFilter = true }: TransactionTableProps) {
  ...
  const transactionsToShow = useMemo(() => {
    ...
    if (defaultType) {
      transactions = transactions.filter((t) => t.type === defaultType);
    }
    return sortTransactionsByDate(transactions);
  }, [filteredTransactions, defaultType]);
